  ...
  return (
    <svg viewBox="0 0 260 260" style={{ width: '100%', maxWidth: '260px' }}>
      ...
      {[0.25, 0.5, 0.75, 1].map((l, i) => (
        <polygon key={i} points={poly(l)} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
      ))}
      {/* Axis lines */}
      {axes.map((_, i) => {
        const p = pt(i, 1)
      ...
      {/* Score fill */}
      <polygon points={scoreD} fill="rgba(99,102,241,0.12)" stroke="#6366f1" strokeWidth="1.5" strokeLinejoin="round" />
      {/* Score dots */}
      {scores.map((s, i) => {
        const p = pt(i, s)
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', overflow: 'visible' }}>
      ...
      {data.map((v, i) => (
        <circle key={i} cx={x(i)} cy={y(v)} r={i === data.length - 1 ? 3.5 : 2}
          fill={i === data.length - 1 ? '#6366f1' : 'rgba(99,102,241,0.5)'} />
      ))}
    </svg>
  )
}
...
const bars = [
  { label: 'Memoria de trabajo',    value: 78, color: '#6366f1' },
  { label: 'Velocidad de reaccion', value: 74, color: '#3b82f6' },
      ...
      {bars.map((bar, i) => (
        <div key={bar.label}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: '12px' }}>{bar.label}</span>
            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '12px' }}>{bar.value}%</span>
          </div>
...
const metricCards = [
  {
    label: '01',
    title: 'Velocidad cognitiva',
    desc: 'Medimos el tiempo de respuesta ante estímulos visuales para evaluar la agilidad de procesamiento de tu mente.',
    color: '#6366f1',
  },
  {
    label: '02',
    title: 'Memoria de trabajo',
    desc: 'Evaluamos cuánta información puedes retener y manipular activamente en un período corto de tiempo.',
    color: '#3b82f6',
  },
  {
    label: '03',
    title: 'Estado emocional',
    desc: 'A través de cuestionarios clínicos validados detectamos tu nivel de ansiedad y bienestar emocional.',
    color: '#10b981',
  },
  {
    label: '04',
    title: 'Concentracion',
    desc: 'Tareas de atención selectiva que miden tu capacidad para ignorar distractores y mantener el foco.',
    color: '#f59e0b',
  },
]
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px' }}>
        ...
        >
          ...
          <h2 style={{
            ...
            Como medimos<br />
            <span style={{ color: 'rgb(107,114,128)' }}>tu rendimiento mental</span>
          </h2>
          <p style={{ marginTop: '16px', color: 'rgb(107,114,128)', fontSize: '17px', lineHeight: 1.7, maxWidth: '500px' }}>
          ...
          {metricCards.map((card, i) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                ...
                <span style={{ color: 'rgba(255,255,255,0.12)', fontSize: '12px', fontWeight: 600, letterSpacing: '1px' }}>
                  {card.label}
                </span>
              </div>
              <h3 style={{ color: 'white', fontSize: '15px', fontWeight: 600, margin: '0 0 10px' }}>
                {card.title}
              </h3>
        ...
        >
          ...
          <div style={{
            ...
            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '10px', letterSpacing: '2.5px', textTransform: 'uppercase' }}>
              Perfil cognitivo
            </span>
            <h3 style={{ color: 'white', fontSize: '15px', fontWeight: 600, margin: '10px 0 24px' }}>
              Mapa de fortalezas
            </h3>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <RadarChart />
            </div>
            <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '11px', textAlign: 'center', margin: '16px 0 0' }}>
          ...
          <div style={{ flex: '1.4', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                <div>
                  ...
                  <h3 style={{ color: 'white', fontSize: '15px', fontWeight: 600, margin: '8px 0 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
                {['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8'].map(s => (
                  <span key={s} style={{ color: 'rgba(255,255,255,0.2)', fontSize: '10px' }}>{s}</span>
                ))}
              </div>
            </div>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', fontSize: '15px', fontWeight: 600, margin: '10px 0 20px' }}>
                Puntuacion por area
              </h3>
              <BarRows />
            </div>
